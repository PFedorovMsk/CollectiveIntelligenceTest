...
enum PlottingHint {
    ...
    ,
    phFastPolylines =
        0x001 ///< <tt>0x001</tt> Graph/Curve lines are drawn with a faster method. This reduces the
...
enum SelectionRectMode {
    srmNone ///< The selection rect is disabled, and all mouse events are forwarded to the
            ///underlying objects, e.g. for
            /// axis range dragging
    ,
    srmZoom ///< When dragging the mouse, a selection rect becomes active. Upon releasing, the axes
            ///that are currently
    /// set as range zoom axes (\ref QCPAxisRect::setRangeZoomAxes) will have their ranges zoomed
    /// accordingly.
    ,
    srmSelect ///< When dragging the mouse, a selection rect becomes active. Upon releasing,
              ///plottable data points that
    /// were within the selection rect are selected, if the plottable's selectability setting
    /// permits. (See
    ...
    srmCustom ///< When dragging the mouse, a selection rect becomes active. It is the programmer's
              ///responsibility to
    /// connect according slots to the selection rect's signals (e.g. \ref
    /// QCPSelectionRect::accepted) in order
    ...
    } else // don't need to prepend, so append and merge if necessary
    {
        ...
        if (oldSize > 0 &&
            !qcpLessThanSortKey<DataType>(*(constEnd() - n - 1),
                                          *(constEnd() - n))) // if appended range keys aren't all
                                                              // greater than existing ones, merge
                                                              // the two partitions
...
void QCPDataContainer<DataType>::add(const QVector<DataType> &data, bool alreadySorted)
{
    ...
    if (alreadySorted && oldSize > 0 &&
        !qcpLessThanSortKey<DataType>(*constBegin(),
                                      *(data.constEnd() - 1))) // prepend if new data is sorted and
                                                               // keys are all smaller than or equal
                                                               // to existing ones
    {
        if (mPreallocSize < n)
            preallocateGrow(n);
...
void QCPDataContainer<DataType>::removeBefore(double sortKey)
{
    ...
    mPreallocSize +=
        itEnd -
        it; // don't actually delete, just add it to the preallocated block (if it gets too large,
...
public:
    ...
    void setBrush(const QBrush &brush);
    void
    setScatterStyle(const QCPScatterStyle &            scatterStyle,
...
public:
    ...
    // introduced virtual methods:
    virtual double selectTest(const QPointF &pos, bool onlySelectable,
                              QVariant *details = 0) const override = 0;
                                                    ...
                                                    QVariant *details) const
{
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it) {
        ...
        if (keyRange.contains(mainKey) &&
            valueRange.contains(mainValue)) // make sure data point is inside visible
    ...
    if (mParentPlot->plottingHints().testFlag(QCP::phFastPolylines) &&
        ...
        while (i < lineDataSize) {
            if (qIsNaN(lineData.at(i).y()) || qIsNaN(lineData.at(i).x()) ||
                qIsInf(lineData.at(i).y())) // NaNs create a gap in the line. Also filter Infs which
                                            // make drawPolyline block
            {
